...
if not st.session_state.liberado_pago:
    ...
    st.sidebar.markdown("---")
    st.sidebar.info(
        f"🎯 Testes gratuitos restantes: **{restantes} de 3**"
    )
...
if modulo == "📊 Visão Geral e BDI":
    ...
    with col1:
        ...
        qtd_cozinhas = st.number_input(
            "Número de Cozinhas:", min_value=1, value=1, step=1, key="hid_coz"
        )
    with col2:
    ...
    with col2:
        ...
        taxa_perda_piso = st.slider(
            "Taxa de Perda de Recorte de Piso (%):", 5, 20, 10, key="rev_perda"
        )
    ...
    custo_total_obra = st.number_input(
        "Custo Total Estimado da Obra (R$):", value=150000.0, key="curva_val"
    )
    ...
    with col2:
        ...
        prazo_meses = st.number_input(
            "Prazo de Execução (meses):", min_value=1, value=6, key="ct_mes"
        )
...
# ==========================================
    ...
    # Se já estourou o limite e não está pago, exibe o bloqueador global direto na IA
    if not st.session_state.liberado_pago and st.session_state.contador_acessos_geral >= 3:
        renderizar_box_pagamento_global()
    else:
        ...
        if prompt_usuario := st.chat_input(
            "Ex: 'Quantos blocos gastam no muro?' ou 'Qual o traço de concreto"
            " ideal?'"
        ):
            # Validação e consumo de acesso ao enviar mensagem para a IA
            if not st.session_state.liberado_pago:
                if st.session_state.contador_acessos_geral >= 3:
                    st.rerun()
                else:
            ...
            if not GEMINI_API_KEY:
                st.error(
                    "⚠️ A `GEMINI_API_KEY` não foi encontrada nos Secrets do"
                    " Streamlit."
                )
            else:
                ...
                with st.chat_message("assistant"):
                    with st.spinner(
                        ...
                        prompt_sistema = (
                            ...
                            " expressões de canteiro de obras (como 'fala, meu"
                            " irmão', 'na lata'). Especialista em construção"
                            " civil brasileira."
                        )
                        ...
                        try:
                            ...
                            payload = {
                                "contents": [{"parts": [{"text": conteudo_prompt}]}]
                            }
                            ...
                            if response.status_code == 200:
                                ...
                                try:
                                    ...
                                    resposta_ia = (
                                        "⚠️ Opa, meu irmão! A resposta veio em"
